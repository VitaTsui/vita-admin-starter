import {
  ParamData,
  createParam,
  editParam,
  getParam,
} from "@/services/apis/sysmgmt/param";

import FormModalStore from "@/stores/basisStoreClass/FormModalStore";
import { ResType } from "@/services/Axios";
import { makeObservable } from "mobx";

class ParamFormStore extends FormModalStore<ParamData> {
  constructor() {
    super();
    makeObservable(this);
  }

  /**
   * 获取详情
   * @param id
   */
  protected _getFormData = (id: number | string) => {
    getParam(id).then((res) => {
      if (res.code === 0) {
        this._formData = res.data;
      } else {
        this._message(res);
      }
    });
  };

  /**
   * 新增
   * @param data
   * @param fn
   */
  public addFormData = (data: ParamData, fn?: (res: ResType) => void) => {
    createParam(data).then((res) => {
      if (res.code === 0) {
        fn?.(res);
      }

      this._message(res);
    });
  };

  /**
   * 编辑
   * @param id
   * @param data
   * @param fn
   */
  public editFormData = (
    id: number | string,
    data: ParamData,
    fn?: (res: ResType) => void
  ) => {
    data.id = id;

    editParam(data).then((res) => {
      fn?.(res);

      this._message(res);
    });
  };
}

export default new ParamFormStore();
