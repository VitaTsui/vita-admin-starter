...
路径别名：`@/*` → `src/*`；其中 **`@/components/*` 重定向到 `@hsu-react/ui`**
（见 `tsconfig.json` 与 `config/webpack.config.common.cjs`）。
...
（已基于 Ant Design 5 做二次封装并统一观感）。`@/components/*` 别名直接指向该库：

```tsx
import Panel from "@/components/Panel"; // → @hsu-react/ui
...
└─ src/
   ...
   ├─ components/        # （别名）指向 @hsu-react/ui 组件库（Panel / Form / Table / Chart …）
